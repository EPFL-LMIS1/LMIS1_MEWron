import g_code_pipe_Sync as pipe
import constants
import datetime
import sys
...
layers = 3      #This is the number of layers for the channels to be printed
spacing = 0.4  #This is the distance in between the fibres for channels
#you need to duplicate the variable for multiple print in the same g-code file

repetition_distance = length/(constants.PIVOT_points+1) #this is the number of repeating units, defines the number of pivots created in tubular encapsulation to hold the pattern
...
starting_position=pipe.position(15, 41.5, 2.5) #position in which the polymer aderes to the shaft
stabilisation_pos=pipe.position(17, 41.5, 2.5) #start position of the stabilisation pattern
init_position=pipe.position(25,41.5,2.5) #starting position of the pattern
end_pos=pipe.position(25,41.5,2.5) #the position of the end of the pattern
...
header =";" + author + "\n;" + str(today) + "\n;"\
        +"Pattern = "+pattern+"\n;"\
        +"Length = " +str(length) + "   Repetition distance = " + str(repetition_distance) +"\n;"\
...
match pattern:
    ...
    case "channel":
        pattern_sequence = pipe.allignement_channels(init_position, length, repetition_distance, layers, spacing)
    case "custom":
